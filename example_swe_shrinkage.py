# -*- coding: utf-8 -*-
"""
Shallow Water Equations on the sphere — extended experiment with the
three new precision-space shrinkage criteria.
...
introduced in the paper. The earlier file ``example_swe.py`` covered
the baseline comparison without these three.
...
Configuration:
  ...
  Assimilation:         every 6 hours during 7 days  =>  29 cycles
  Observation noise:    isotropic Gaussian (per-variable scale)
  Observations:         a continental mask is generated for the sphere; only
                        components inside the mask are observed. All three
                        fields (u, v, h) are observed at the same locations.
  Methods:              8 — LETKF, LEnKF, EnKF-MC, EnKF-LW, EnKF-RBLW,
                        and the three new EnKF-Sh-Binv-{MSE,Stein,DA}.
  Snapshots:            ensemble stored at 0%, 25%, 50%, 75%, 100%.

The cached spin-up artefacts live in ./swe_cache/ (reusable from the
baseline run). Plots are written under ./swe_shrinkage_figs/.
...
#   (1) localised baselines (R-localisation),
#   (2) covariance-space shrinkage (Niño-Ruiz, Guzman, Jabba 2021),
#   (3) precision-space shrinkage with three principled criteria (this paper).
#
# Notes:
#  - EnKF-BLoc is omitted at this resolution because the dense
#    decorrelation matrix L (n × n) does not fit in memory for LMAX=21.
#  - EnKF-Shrinkage-Binv (the heuristic γ/(1+γ) shrinkage on the
#    precision) is omitted: it is empirically unstable here; see the
#    three principled criteria below.
#  - All precision-space shrinkages use modified-Cholesky target with
#    r={u:4, v:4, h:4} and truncated-SVD pseudo-inverse with rtol=0.05.
METHODS = {

    # (3) Precision-space shrinkage — three principled criteria (new paper)
    "EnKF-Sh-Binv-Stein":  dict(method="enkf-shrinkage-binv-stein",
                                  r={"u": 4, "v": 4, "h": 4},
                                  rtol_pseudo_inverse=0.1),
